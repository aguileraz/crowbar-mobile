...
if [ -f "../android/scripts/run-tests.sh" ]; then
    test_pass
else
...
if [ -f "../android/scripts/parse-jest-results.sh" ]; then
    test_pass
else
...
if [ -f "../android/scripts/run-detox-tests.sh" ]; then
    test_pass
else
...
if [ -f "../android/scripts/retry-handler.sh" ]; then
    test_pass
else
...
for script in ../android/scripts/run-tests.sh \
              ../android/scripts/parse-jest-results.sh \
              ../android/scripts/run-detox-tests.sh \
              ../android/scripts/retry-handler.sh; do
...
if [ -f "../android/scripts/run-tests.sh" ] && \
   grep -q "unit\|integration\|e2e" ../android/scripts/run-tests.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "../android/scripts/parse-jest-results.sh" ] && \
   grep -q "json\|JSON" ../android/scripts/parse-jest-results.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "../android/scripts/run-detox-tests.sh" ] && \
   grep -q "headless\|--headless" ../android/scripts/run-detox-tests.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "../android/scripts/retry-handler.sh" ] && \
   grep -q "MAX_RETRIES\|RETRY_COUNT" ../android/scripts/retry-handler.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "../android/scripts/aggregate-results.sh" ]; then
    test_pass
else
...
if [ -f "../android/scripts/run-tests.sh" ] && \
   grep -q "parallel\|PARALLEL\|concurrent" ../android/scripts/run-tests.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "../android/scripts/run-tests.sh" ] && \
   grep -q "mkdir.*results\|OUTPUT_DIR" ../android/scripts/run-tests.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "../android/scripts/run-tests.sh" ] && \
   grep -q "exit.*\$\|return.*\$" ../android/scripts/run-tests.sh 2>/dev/null; then
    test_pass
else
...
if [ -f "./test-execution-integration.sh" ]; then
    test_pass
else
